...
app.get("/users", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.user.role === "superadmin") {
      ...
      }).select("_id name serviceNumber role status");
    }
    res.json({ users });
  } catch (err) {
...
app.get("/api/notices", authMiddleware, async (req, res) => {
  try {
    const allNotices = await Notice.find({ organizationId: req.user.orgId })
      .populate(
        "authorId",
        "name rank role promoToIlbyung promoToSangbyung promoToByungjang"
      )
      .populate(
        "comments.userId",
        "name rank role promoToIlbyung promoToSangbyung promoToByungjang"
    ...
    const sortedNotices = allNotices.sort((a, b) => {
      const aIsSticky =
        a.isImportant &&
        today - new Date(a.createdAt) < expiryDays * 24 * 60 * 60 * 1000;
      const bIsSticky =
        b.isImportant &&
        today - new Date(b.createdAt) < expiryDays * 24 * 60 * 60 * 1000;
    ...
    res.json({
      success: true,
      role: req.user.role,
      currentUserId: req.user.userId || req.user._id,
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      const authorStr = notice.authorId._id
        ? notice.authorId._id.toString()
        : notice.authorId.toString();
      const reqUserStr = (req.user.userId || req.user._id).toString();

      if (
        authorStr !== reqUserStr &&
        !["superadmin", "admin", "approver"].includes(req.user.role)
      ) {
      ...
      notice.isImportant = req.body.isImportant === "true";
      if (req.files && req.files.length > 0)
        notice.attachedFiles = req.files.map((f) => `/uploads/${f.filename}`);
...
// ==========================================
app.put(
  "/api/notices/:noticeId/comment/:commentId",
  authMiddleware,
  async (req, res) => {
    try {
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!GalleryModel)
        return res.status(500).json({ error: "Gallery Model Error" });
...
app.put(
  "/api/gallery/:galleryId/comment/:commentId",
  authMiddleware,
  async (req, res) => {
    try {
      const GalleryModel = mongoose.models.Gallery;
...
app.post("/api/notices/:id/like", authMiddleware, async (req, res) => {
  try {
    ...
    if (index > -1) notice.likes.splice(index, 1);
...
// ==========================================
// 🔔 [升級版] 小鈴鐺通知 (🔥 已經補上 APPROVED 與 FORCE_CANCELLED)
// ==========================================
app.get("/leaves/notifications", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    recentImportantNotices.forEach((notice) => {
      const authorStr =
        notice.authorId && notice.authorId._id
          ? notice.authorId._id.toString()
          : notice.authorId?.toString();
...
app.get("/api/omni-search", authMiddleware, async (req, res) => {
  try {
    ...
    const matchedUsers = await User.find({
      ...
      .select(
        "_id name rank role promoToIlbyung promoToSangbyung promoToByungjang status"
      )
      .lean();
    ...
    const usersToDisplay = matchedUsers
